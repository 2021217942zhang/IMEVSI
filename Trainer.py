...
class Model:
    ...
    def hr_inference_sequence(self, img0=None, img1=None, af_down=None, af=None, down_scale = 1.0, flow = None, mask = None):
        if down_scale!=1:
            ...
            self.flow, self.mask, _ ,self.af_down = self.net.calculate_flow_inference(img0_down, img1_down, af_down, flow, mask)
            flow = F.interpolate(self.flow, scale_factor = 1/down_scale, mode="bilinear", align_corners=False) * (1/down_scale)
            mask = F.interpolate(self.mask, scale_factor = 1/down_scale, mode="bilinear", align_corners=False)
